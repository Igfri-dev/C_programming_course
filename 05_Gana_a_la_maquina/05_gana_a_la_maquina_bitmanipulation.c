...
void print_table(uint32_t played_letters, uint32_t ownership_flags, uint8_t lives)
{
    ...
    int col = 0;  // Track current column position
    for (char c = MIN_CHAR; c <= MAX_CHAR; c++)
    {
        uint32_t bit = 1U << (c - MIN_CHAR);  // Create bitmask for current letter
        char display = c;  // Default to showing the letter
...
int main()
{
    ...
    while ((game_data & (USER_WON | COMPUTER_WON)) == 0 && (game_data & 0b00001111) > 0)
    {
        ...
        while (1)
        {
            ...
            letter_bit = 1U << (user_input - MIN_CHAR);  // Generate bitmask for input
        ...
        do
        {
            ...
            letter_bit = 1U << (computer_guess - MIN_CHAR);  // Convert to bitmask
